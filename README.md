...
<<自媒体文章生成、排版与发布推送工具（赛道制）—— AI写稿 + 自动配图 + 内容排版 + Web控制台

> 🐛 **v2.2** — 本轮加固了 Web 控制台默认监听和文章路径校验，修复每日多篇生成、并发触发、Emoji 误判、Markdown 图片解析等问题，并将正文插图调整为 2 张三等分布局。

## ✨ 功能亮点

<<- 🏁 **多赛道制**：支持多个赛道文章撰写，每个赛道独立prompt`n- 🔍 **每日热门选题**：自动搜索热点话题，AI筛选最佳切入点`n- ✍️ **AI生成文章**：Qwen3-235B 写作，字数精准`n- 🖼 **AI自动配图**：Kolors文生图，封面（2.35:1）+ 2张横向插图（16:9），中文prompt自动翻译英文`n- 🎨 **内容精美排版**：紫色系排版，行间距2.2，自动加粗节奏，无底色，复制即用`n- 🌐 **Web控制台**：在线浏览、手机预览、一键复制HTML到编辑器
...
<<| **文章预览** | 手机框架预览排版效果，一键复制排版 |`n| **封面下载** | 一键下载2.35:1封面图 |
| **赛道设置** | 开关赛道、调整每日篇数 |
| **定时设置** | 修改每天自动生成时间 |
| **生成设置** | 切换图片来源、调整字数范围 |
| **立即生成** | 手动触发指定赛道文章生成 |
...
<<- **零底色**：所有文字元素无background，复制到富文本编辑器不带入任何颜色底色`n- **编辑器兼容**：不用::after、border-image、background-clip等不兼容属性`n- **图片base64内嵌**：复制排版时图片自动上传，不依赖外部图床
- **2张横向插图**：16:9比例，分别靠近正文 1/3、2/3 位置，将文章自然分成三段
- **首字下沉**：文章首段首字放大紫色，增加视觉层次
- **装饰分割线**：SVG内联分割线，编辑器兼容
...
| 图片类型 | 尺寸 | 比例 | 说明 |
|----------|------|------|------|
<<| 封面图 | 900×383 | 2.35:1 | 内容平台封面比例 |
| 插图 | 1024×576 | 16:9 | 横向长方形，正文配图 |
| 引导图 | 原图裁剪 | - | 开头关注+结尾点赞在看 |

- 图片生成模型：Kwai-Kolors/Kolors
...
3. **打开Web控制台** → 浏览今日文章
4. **手机预览** → 确认排版效果
<<5. **复制HTML** → 一键复制，粘贴到编辑器`n6. **上传封面** → 下载封面图，上传到后台
7. **发布** 🎉

## ⚠️ 平台复制注意事项

- 复制HTML时，图片已base64内嵌，粘贴后平台会自动上传到服务器
- 所有文字元素无底色，不会带入任何背景颜色
- 封面图不包含在正文中，需单独上传到内容平台后台
- 正文包含2张横向插图 + 首尾引导图
